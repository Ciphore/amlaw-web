...
export default async function AttorneyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function fetchOne(u: string): Promise<Attorney | undefined> {
    const r = await fetch(new URL(u), { cache: 'no-store' })
    const raw: unknown = await r.json()
    if (Array.isArray(raw)) {
      const arr = raw as Attorney[]
      return arr.find((x) => x && x.attorney_id === p.id)
    }
    if (raw && typeof raw === 'object' && Array.isArray((raw as { hits?: unknown }).hits)) {
      const hits = (raw as { hits: unknown }).hits as Attorney[]
      return hits.find((x) => x && x.attorney_id === p.id)
    }
    return raw as Attorney | undefined
  }

  const byAttorneyId = await fetchOne(`${absoluteBase()}/search?attorney_id=${encodeURIComponent(p.id)}&limit=1`)
  const a = byAttorneyId ?? (await fetchOne(`${absoluteBase()}/search?id=${encodeURIComponent(p.id)}&limit=1`))
